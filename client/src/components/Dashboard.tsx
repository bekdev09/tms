...
import { useCreateSubmissionMutation, useGetSubmissionsQuery } from '../store/api/dataApi';
...
export default function Dashboard() {
  ...
  const deleteSubmission = async (id: string) => {
    try {
      await supabase.from('data_submissions').delete().eq('id', id);
      refetch();
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 transition-colors">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {textInputs.length < 5 && (
                    <button
                      ...
                      className="w-full px-4 py-3 text-sm bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-xl font-medium shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5"
                    >
                      + Add Input
                    </button>
                  )}
              ...
              {message && (
                <div className={`p-4 rounded-xl text-sm ${message.includes('success')
